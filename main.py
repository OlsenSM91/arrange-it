...
def organize_photos_by_team(csv_path, image_directory):
    ...
    df['photo'] = df['photo'].apply(lambda x: x.strip())
    ...
    for index, row in df.iterrows():
        ...
        photo_path = next((f for f in os.listdir(image_directory) if f.lower() == photo_name.lower()), None)
        if photo_path:
            source_path = os.path.join(image_directory, photo_path)
            destination_path = os.path.join(team_directory, photo_name)
            shutil.move(source_path, destination_path)
        else:
            print(f"File {photo_name} not found in {image_directory}")
